# 🔖 트위터 북마크 → Notion 자동 정리 파이프라인

트위터(X)에 쌓인 북마크를 자동으로 수집하고, 카테고리별로 분류해서 Notion 데이터베이스에 저장해주는 도구입니다.

> 개발 지식 없어도 사용할 수 있습니다. 설정은 최초 1회만 하면 됩니다.
...
- 드로잉 팁 / 팬아트 / 게임 공략 / 기술·AI 등 카테고리로 자동 분류합니다
...
| Gemini API 키 | 선택 | "기타" 항목 AI 재분류 시에만 필요 |
...
> 💡 토큰이 만료되면 (로그아웃하거나 비밀번호 변경 시) setup.bat을 다시 실행해서 갱신하면 됩니다.
...
"기타"로 분류된 트윗이 너무 많다면, Gemini AI가 다시 분류해줄 수 있습니다.

#### Gemini API 키 받는 방법 (무료)

1. https://aistudio.google.com/app/apikey 접속 (구글 계정 로그인)
2. **"+ API 키 만들기"** 클릭 → 복사
3. `setup.bat` 다시 실행해서 마지막 항목에 입력

> 무료 플랜으로 충분합니다 (분당 15회 제한).
